...
export async function applyAgentConfig(params: {
  ...
  if (isSisyphusEnabled && builtinAgents.sisyphus) {
    ...
    const preferredDefaultAgent = params.pluginConfig.default_run_agent;
    if (
      typeof preferredDefaultAgent === "string" &&
      preferredDefaultAgent.length > 0 &&
      (params.config.agent as Record<string, unknown>)[preferredDefaultAgent]
    ) {
      (params.config as { default_agent?: string }).default_agent = preferredDefaultAgent;
    }
  }
