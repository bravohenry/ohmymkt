 ...
 * [OUTPUT]: createContentWriterAgent() — content creation subagent config
 * [POS]: Subagent for text/image/video content creation, delegated to by growth-manager
 * [PROTOCOL]: Update this header when changed, then check CLAUDE.md
 */

import type { AgentConfig } from "@opencode-ai/sdk";

export function createContentWriterAgent(model: string): AgentConfig {
  return {
    description: "Content creation — text copy, image generation, video production for marketing assets",
    mode: "subagent",
    model,
    temperature: 0.3,
    prompt: `# Content Writer — Creative Content Subagent
...
- \`ohmymkt_research_brief\` (read) — access research findings
- \`ohmymkt_save_positioning\` (read) — access selected positioning angle
- \`ohmymkt_asset_manifest\` — track all created assets
- \`ohmymkt_generate_image\` — generate images via configured provider
- \`ohmymkt_generate_video\` — generate videos via Remotion or AI
- \`ohmymkt_provider_config\` (read) — check available providers
