...
|---|---|---|
| requirements-analyst | Requirement decomposition and scope clarity | Before plan drafting; ambiguous goals; conflicting constraints |
| plan-reviewer | Plan quality and risk validation (Metis-like) | Before launch; before large channel investment; after major plan change |
| execution-manager | Execution routing and specialist dispatch | After plan approval; day-to-day campaign operations; cycle execution |
...
All specialist work is delegated to sub-agents. Planning quality flows through `requirements-analyst` + `plan-reviewer`; operational delivery flows through `execution-manager`.
