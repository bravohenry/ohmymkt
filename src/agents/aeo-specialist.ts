 ...
 * [OUTPUT]: createAeoSpecialistAgent() — AEO subagent config
 * [POS]: Subagent for Answer Engine Optimization, delegated to by growth-manager
 * [PROTOCOL]: Update this header when changed, then check CLAUDE.md
 */

import type { AgentConfig } from "@opencode-ai/sdk";

export function createAeoSpecialistAgent(model: string): AgentConfig {
  return {
    description: "Answer Engine Optimization — LLM visibility, structured data, citation signals",
    mode: "subagent",
    model,
    temperature: 0.1,
    prompt: `# AEO Specialist — Answer Engine Optimization Subagent
