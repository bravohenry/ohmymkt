# Contributing to ohmymkt

First off, thanks for taking the time to contribute! This document provides guidelines and instructions for contributing to ohmymkt.
...
git clone https://github.com/bravohenry/ohmymkt.git
cd ohmymkt
...
2. **Update your OpenCode config** (`~/.config/opencode/opencode.json` or `opencode.jsonc`):
   ...
   {
     "plugin": [
       "file:///absolute/path/to/ohmymkt/dist/index.js"
     ]
   }
   ```
   
   For example, if your project is at `/Users/yourname/projects/ohmymkt`:
   ```json
   {
     "plugin": [
       "file:///Users/yourname/projects/ohmymkt/dist/index.js"
     ]
   }
   ```

   > **Note**: Remove `"ohmymkt"` from the plugin array if it exists, to avoid conflicts with the npm version.
...
│   └── index.ts       # Main plugin entry (plugin factory)
...
Thank you for contributing to ohmymkt! Your efforts help make AI-assisted coding better for everyone.
