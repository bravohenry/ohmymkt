 ...
 * [OUTPUT]: createSeoEngineerAgent() — technical SEO subagent config
 * [POS]: Subagent for technical SEO execution, delegated to by growth-manager
 * [PROTOCOL]: Update this header when changed, then check CLAUDE.md
 */

import type { AgentConfig } from "@opencode-ai/sdk";

export function createSeoEngineerAgent(model: string): AgentConfig {
  return {
    description: "Technical SEO execution — site structure, Schema markup, Core Web Vitals, crawl budget",
    mode: "subagent",
    model,
    temperature: 0.1,
    prompt: `# SEO Engineer — Technical SEO Subagent
